...
{
public:
	REFERENCE_TIME			rtStart_10mhz;				///< 10 Mhz start time
	REFERENCE_TIME			rtStop_10mhz;				///< 10 Mhz stop time
	BYTE					*data;						///< Packet payload
	int						size;						///< Actual data size

	// extended
	bool					timestamp_ex;				///< Is extended time stamp information available?
	__int64					pts;						///< PTS value read from IMediaSampleEx interface
	__int64					dts;						///< DTS value read from IMediaSampleEx interface
	int						clock;						///< Clock value read from IMediaSampleEx interface

	bool					sync_point;					///< Is this packet a sync point ?
	bool					has_time;					///< Is the time stamp information available ?
	int						stream_no;					///< associated stream index
	bool					is_eos;						///< Is this an end of stream packet ?
	...
	int						picture_type;				///< Picture coding type helper 
...
{
public:
	int						index;						///< Index of the stream in the output multiplex
	bool					is_interleaved;				///< Is this stream interleaved ?
	bool					active;						///< Is it active (connected) ?
	int						size;						///< Upper limit of the sample queue size
	void					*data;						///< Associated user data (pin)

	CMuxInterPacketList		queue;						///< Buffer queue
	CAMEvent				ev_can_write;				///< Is the queue writable ?
	CCritSec				lock;						///< Access lock
...
{
public:
	CMuxInterStreamArray	streams;				///< List of input streams
